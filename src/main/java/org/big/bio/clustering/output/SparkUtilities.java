...
/**
 * org.big.bio.clustering.output.SparkUtilities
 * User: Steve
 * Date: 11/14/2017
 */
...
import com.lordjoe.distributed.spark.accumulators.AbstractLoggingPairFunction;
...
import parquet.org.slf4j.spi.LoggerFactoryBinder;
import scala.Option;
import scala.Tuple2;
...
//        // set all properties in the SparkProperties file
    ...
    public static class KeyValueObjectToTuple2<K extends Serializable, V extends Serializable> implements FlatMapFunction2<KeyValueObject<K, V>, K, V> {
        @Override
        public Iterator<V> call(final KeyValueObject<K, V> ppk, final K pK) throws Exception {
            Object[] items = {ppk.value};
            return Arrays.asList((V[]) items).iterator();
        }
...
//    }
    ...
    public static <K, V> JavaPairRDD<K, V> persistAndShowHash(@Nonnull final String message, Appendable out, @Nonnull final JavaPairRDD<K, V> inp) {
        ...
        Map<K, Long> keyCounts = ret.countByKey();
        try {
            out.append(message + "\n");
        }
        catch (IOException e) {
...
//    }
    ...
    public static <K, V> void showCounts(JavaPairRDD<K, V> binPairs) {
        Map<K, Long> counts = binPairs.countByKey();
